...
class harajCrawler(CrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(restrict_xpaths='//a[contains(@href,"/11")]'),  follow=True),
        Rule(LinkExtractor(restrict_xpaths='//a[contains(@href,"/tagsr")]'), follow=True),
    ...
    def parse_item(self, response):
        ...
        yield item
        for href in related:
            yield Request(response.urljoin(href),dont_filter=False, callback=self.parse_item)
